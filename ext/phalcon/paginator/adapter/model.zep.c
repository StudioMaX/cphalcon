...
PHP_METHOD(Phalcon_Paginator_Adapter_Model, paginate)
{
	...
	ZEPHIR_CALL_FUNCTION(&_9, "call_user_func", NULL, 275, &_8, &parameters);
	zephir_check_call_status();
	...
	if (rowcount > 0) {
		...
		ZEPHIR_CALL_FUNCTION(&_14$$6, "call_user_func", NULL, 275, &_12$$6, &parameters);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(&items, &_14$$6);
		ZEPHIR_CALL_METHOD(&pageItems, &items, "toarray", NULL, 0);
		zephir_check_call_status();
	}
	next = (pageNumber + 1);
	if (next > totalPages) {
